...
public class EnumButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if ((!FontMod.getInstance().cfont.getValue() && M3dC3t.moduleManager.getModuleByName("CustomFont").isEnabled()) || !M3dC3t.moduleManager.getModuleByName("CustomFont").isEnabled()) {
            M3dC3t.textManager.drawStringWithShadow(this.setting.getName() + " " + ChatFormatting.GRAY + (this.setting.currentEnumName().equalsIgnoreCase("ABC") ? "ABC" : this.setting.currentEnumName()), this.x + 2.3f, this.y - 1.7f - (float) M3dC3tGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        } else {
            M3dC3t.textManager.drawStringClickGui(this.setting.getName() + " " + ChatFormatting.GRAY + (this.setting.currentEnumName().equalsIgnoreCase("ABC") ? "ABC" : this.setting.currentEnumName()), this.x + 2.3f, this.y - 0.7f - (float) M3dC3tGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
