...
public class FriendCommand
    ...
    public void execute(String[] commands) {
        if (commands.length == 1) {
            if (M3dC3t.friendManager.getFriends().isEmpty()) {
                FriendCommand.sendMessage("Friend list empty D:.");
            } else {
                String f = "Friends: ";
                for (FriendManager.Friend friend : M3dC3t.friendManager.getFriends()) {
                    try {
        ...
        if (commands.length == 2) {
            ...
            FriendCommand.sendMessage(commands[0] + (M3dC3t.friendManager.isFriend(commands[0]) ? " is friended." : " isn't friended."));
            return;
        }
        if (commands.length >= 2) {
            switch (commands[0]) {
                case "add": {
                    M3dC3t.friendManager.addFriend(commands[1]);
                    FriendCommand.sendMessage(ChatFormatting.GREEN + commands[1] + " has been friended");
                    return;
                }
                case "del": {
                    M3dC3t.friendManager.removeFriend(commands[1]);
