...
import dev.madcat.m3dc3t.features.setting.EnumConverter;
...
public class ConfigManager implements Util {
    ...
    public String config = "M3dC3t/config/";
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        ...
        M3dC3t.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
    }
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry<String, JsonElement> entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    M3dC3t.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                continue;
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("M3dC3t").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("M3dC3t/" + name + "/"))) {
            this.config = "M3dC3t/" + name + "/";
        } else {
            this.config = "M3dC3t/config/";
        }
        M3dC3t.friendManager.onLoad();
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("M3dC3t").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("M3dC3t/" + name + "/"));
    }
    ...
    public void saveConfig(String name) {
        ...
        if (!path.exists())
            path.mkdir();
        M3dC3t.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        ...
        try {
            if (currentConfig.exists()) {
                ...
                writer.write(tempConfig.replaceAll("M3dC3t", ""));
                writer.close();
            } else {
    ...
    public void init() {
        this.features.addAll(M3dC3t.moduleManager.modules);
        this.features.add(M3dC3t.friendManager);
        String name = loadCurrentConfig();
        loadConfig(name);
        M3dC3t.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            M3dC3t.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
