...
public class MathUtilll
    ...
    public static Vec3d roundVec(Vec3d vec3d, int places) {
        return new Vec3d(MathUtilll.round(vec3d.x, places), MathUtilll.round(vec3d.y, places), MathUtilll.round(vec3d.z, places));
    }
    ...
    public static double lengthSQ(Vec3d vec3d) {
        return MathUtilll.square(vec3d.x) + MathUtilll.square(vec3d.y) + MathUtilll.square(vec3d.z);
    }
    ...
    public static List<Vec3d> getBlockBlocks(Entity entity) {
        ...
        double minX = MathUtilll.round(bb.minX, 0);
        double minZ = MathUtilll.round(bb.minZ, 0);
        double maxX = MathUtilll.round(bb.maxX, 0);
        double maxZ = MathUtilll.round(bb.maxZ, 0);
        if (minX != maxX) {
    ...
    public static Vec3d calculateLine(Vec3d x1, Vec3d x2, double distance) {
        double length = Math.sqrt(MathUtilll.multiply(x2.x - x1.x) + MathUtilll.multiply(x2.y - x1.y) + MathUtilll.multiply(x2.z - x1.z));
    ...
    public static Vec3d extrapolatePlayerPosition(EntityPlayer player, int ticks) {
        ...
        double distance = MathUtilll.multiply(player.motionX) + MathUtilll.multiply(player.motionY) + MathUtilll.multiply(player.motionZ);
