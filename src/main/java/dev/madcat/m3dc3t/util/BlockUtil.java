...
import static dev.madcat.m3dc3t.util.HoleFillUtil.faceVectorPacketInstant;
import static dev.madcat.m3dc3t.util.HoleFillUtil.processRightClickBlock;

public class BlockUtil
        implements Util {
    public static boolean canBlockBeSeen(double x, double y, double z) {
        if (mc.world.rayTraceBlocks(new Vec3d(mc.player.posX, mc.player.posY + (double) mc.player.getEyeHeight(), mc.player.posZ), new Vec3d(x, y + 1.7, z), false, true, false) != null) return false;
        return true;
    }
    ...
    public static void placeCrystalOnBlock(BlockPos pos, EnumHand hand, boolean swing, boolean exactHand, boolean silent) {
        ...
        if (hand == EnumHand.MAIN_HAND && silent && crystal != -1 && crystal != mc.player.inventory.currentItem) {
            AntiCev.mc.player.inventory.currentItem = crystal;
